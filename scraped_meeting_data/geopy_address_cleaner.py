...
def create_list_from_column_data(csv_file, *columns):
    """
...
if __name__ == '__main__':
    ...
    logger.add(log_file_path, rotation=rotation, level="INFO",
               format="[<green>{time: MMM D YYYY HH:mm:ss.SS}</>]|{file} took {elapsed} to execute|<level>{message}</>",
    ...
    try:
        while aq:
            try:
                ...
                logger.success("[+] [meeting]: {} | [address]: {} | [Index]: [{}]", name, cleaned_address,
                            count)
                count += 1
                ...
                address = aq.popleft()
                name = nq.popleft()
                logger.error("[-] [meeting]: {} | [address]: {} <NOT CLEANED> | [Index]: [{}]", name, address,
                             count)
                count += 1
        ...
        logger.error("Exception Raised: {} @ Index: [{}]", e, count)
