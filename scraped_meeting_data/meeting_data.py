...
def meeting_data_scraper(link):
    data = {'name': [], 'address': [], 'city': [], 'state': [], 'zip_code': [], 'details':{'schedule':{'day':[], 'time':[], 'info': []}}}
...
address_ending_strings = ['Ave', 'Blvd', 'Street', 'Dr', 'Ln', 'Rd', 'Road', 'Highway', 'Hwy', 'Pkwy', 'St']

with open(csv_filename, 'r') as f:
    csv_reader = csv.reader(f)
    link_list = csv_parser(csv_reader, 'link')

meeting_data_scraper(link_list[3])

    # header = next(csv_reader)
    # linkIndex = header.index("link")
...
# with open('clean_meetings_data.csv', 'w') as csvfile:

#     csv_writer = csv.writer(csvfile, delimiter='|')
#     # create a new headers list for the cleaned data: 
#     field_names = ['street', 'city', 'state', 'zip_code']